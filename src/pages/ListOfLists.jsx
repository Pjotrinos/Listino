    ...
    <div class="sm:mx-8 md:mx-10 lg:mx-20">
        ...
        <SettingsModal
          ...
          Text={
            sizePrefix == "sm" ? (
              ...
              leangueBook[langueSelect].text_create_list
            )
          }
          buttonColor={"primary"}
          isIconOnly={sizePrefix == "sm" ? true : false}
      ...
      {fetchState === "pending" ? (
        <div className="flex flex-row justify-center">
          {" "}
          <l-ring className="absolute inset-0" size="60" />{" "}
        </div>
      ) : null}
          ...
          lists.map((oneList) => {
            return (
              <ListAsCard
                id={oneList.id}
                key={oneList.id}
                name={oneList.listName}
                imageUrl={oneList.imageLink}
                DeleteList={DeleteList}
                taskNumber={Object.groupBy(oneList.tasks, ({ isDone }) => isDone)?.false.length ?? 0}
                showArchived={IsShowChecked}
                isArchived={oneList.archived}
              />
            );
          })
        )}
